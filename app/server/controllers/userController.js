...
class UserController {
  async registration(req, res, next) {
    ...
    if (candidate) {
      return next(
        ApiError.badRequest('Sorry,but we already have user with this emai'),
      );
    }
  ...
  async login(req, res, next) {
    ...
    if (!user) {
      return next(
        ApiError.internalRequest(`Didn't found user with this email`),
      );
    }

    let comprePassword = bcrypt.compareSync(password, user.password);
    if (!comprePassword) {
